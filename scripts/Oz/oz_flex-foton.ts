...
setVar $oz_foton_version "OZ-PWarp Fast Flex Foton 2.0"
...
addMenu "" "OZ-PWarp Fast Flex  Foton" "OZ-PWarp Fast Flex  Foton Settings" "." "" "Main" FALSE
addMenu "OZ-PWarp Fast Flex  Foton" "Run" "Run Script." "Z" :Menu_Run "" TRUE
addMenu "OZ-PWarp Fast Flex  Foton" "Continuous"  "Continuous" "C" :Menu_Continuous "" FALSE
addMenu "OZ-PWarp Fast Flex  Foton" "Auto-Buy"  "Auto-Buy" "B" :Menu_Buy "" FALSE
addMenu "OZ-PWarp Fast Flex  Foton" "Auto-Return"  "Auto-Return" "R" :Menu_Return "" FALSE
...
gosub :sub_setMenu
gosub :ozHeader
openMenu "OZ-PWarp Fast Flex  Foton"
...
send "' Activating OZ Fast Flex Foton*"
send "|"
waitFor "(?="
gosub :quikstats
...
fileExists $exists $figfile
	if ($exists = 0)
		setVar $nofile 1
		send "'OZ Fast Flex  Foton - No fig data found - refreshing*"
		gosub :refresh_figs
	else
		gosub :get_da_figs
	end
...
setVar $run "on"
:hoho5
echo ansi_9 "* -=-=-=-=-=-=-=-=-=- OZ Fast Flex  Foton -=-=-=-=-=-=-=-=-=- *"
...
:turn_off
	...
	setVar $run "off"
	send "'OZ Fast Flex  Foton - switching to passive mode.*"
	goto :hoho5
...
:turn_on
	...
	setVar $ptorps $quikstats[$h[11]]
	if ($ptorps = 0)
		send "'OZ Fast Flex  Foton - no photons - remaining in passive mode.*"
		setVar $run "off"
		goto :hoho5
	end
	waitFor "(?="
	...
	if ($citter <> "Citadel")
		send "'OZ Fast Flex  Foton - not at cit prompt - switching to passive mode.*"
		setVar $run "off"
		goto :hoho5
	end
	setVar $run "on"
	send "'OZ Fast Flex  Foton - switching to active mode.*"
	goto :hoho5
...
:pwpfig
	...
	setVar $m 1
	gosub :fighit
	if ($person = $traders[$m]) or ($fercheck = "Ferrengi") or ($fercheck = "Alien")
		goto :hehe5
	end
	if (SECTOR.WARPCOUNT[$sector] <> 2) and (SECTOR.WARPCOUNT[$sector] <> 3)
		goto :hehe5
	end
:pwp_id_check
...
:gotem
	killAllTriggers
	subtract $ptorps 1
	send "' OZ Fast Flex  Foton Fired - Sector -> " $sector "!*"
	...
	elseif ($ptorps = 0) and ($autobuy = "OFF")
		send "'OZ Fast Flex  Foton - out of P-Missles - switching to passive mode.*"
		setVar $run "off"
		goto :hoho5
	end
	if ($continuous = "ON")
...
:nohomelock5
	killAllTriggers
	send "'OZ Fast Flex  Foton - PWarp Lock Failed - switching to pasive mode.*"
	setVar $run "off"
	goto :hoho5
...
uppercase $person
getWord CURRENTLINE $fercheck 6
stripText $sector ":"
...
:refresh_figs
	...
	elseif ($location = "Command")
		...
		send "'OZ Fast Flex  Foton - Fig Refresh must be run from Citadel or Command Prompt*"
		if ($nofile = 1)
			killAllTriggers
			send "'OZ Fast Flex  Foton - Refresh Error - Halting.*"
			halt	
		end
		goto :hehe5
	end
...
:refresh
	send "'OZ Fast Flex  Foton - Refreshing figs - one moment please...*"
	send "g"
...
:doneglist
	...
	send "'OZ Fast Flex  Foton - Fig Count - " $total_figs " sector(s) - " $fig_count "%*"
...
:no_planet
	killAllTriggers
	send "'OZ Fast Flex  Foton - Landing Error - Halting.*"
	halt
...
:ok_beam
	killAllTriggers
	divide $distance1 3
	if ($quikstats[$h[3]] > $distance1)
		send "y"
		goto :port_sd
	else
		send "'OZ Fast Flex  Foton - No ore to reload photons - switching to passive mode.*"
		setVar $run "off"
		goto :hoho5
	end
...
:no_twarp
	killAllTriggers
	send "znl" $planet "*c"
	send "'OZ Fas Flex t Foton - No twarp to reload photons - switching to passive mode.*"
	setVar $run "off"
	goto :hoho5
...
:no_fedlock
	killAllTriggers
	send "'OZ Fast Flex  Foton - No Fed beacon to reload photons - switching to passive mode.*"
	setVar $run "off"
	goto :hoho5
...
:no_twarp_home
	KillAllTriggers
	send "n"
	send "'OZ Fast Flex  Foton - No twarp beacon at base - switching to passive mode.*"
	setVar $run "off"
	goto :hoho5
...
:twarp_home_lock
	...
	if ($phots_buy = 0)
		send "'OZ Fast Flex  Foton - out of photons - switching to passive mode*"
		setVar $run "off"
		goto :hoho5
	end
	waitFor "Citadel command (?"
	...
	setVar $ptorps $quikstats[$h[11]]
	if ($quikstats[$h[2]] < 50)
		send "'OZ Fast Flex  Foton - less then 50 turns - switching to passive mode.*"
		setVar $run "off"
	end
	goto :hoho5
:cant_buy
	send "'OZ Fast Flex  Foton - algn/xp conflict - switching to passive mode.*"
	setVar $run "off"
	goto :hoho5
