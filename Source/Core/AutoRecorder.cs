...
namespace TWXProxy.Core
{
    ...
    public class AutoRecorder
    {
        ...
        /// <summary>
        /// The sector number most recently seen in a command prompt. Sector-display
        /// headers can be generated by scans, ZTM, database displays, or previews,
        /// so they are recorded as observations but are not authoritative player
        /// movement.
        /// </summary>
        ...
        public void RecordLine(string line, string? ansiLine = null)
        {
            ...
            // ── Sector display header ─────────────────────────────────────────
            {
                ...
                if (m.Success && int.TryParse(m.Groups[1].Value, out int sn))
                {
                    ...
                    // Sector-display headers are observations, not proof that the
                    // player moved. ZTM and other scan/database outputs can show many
                    // sectors while the player remains at the same prompt. The current
                    // sector and landed planet location are updated only from prompts
                    // or explicit planet detail/list output.
                    GlobalModules.AutoRecorderDebugLog($"[AutoRecorder] Sector display observed {sn}; current sector remains {_currentSector}\n");
